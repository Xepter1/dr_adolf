...
export async function sendDoiEmail(
  payload: Payload,
  args: { to: string; brandName: string; confirmUrl: string },
): Promise<void> {
  const html = shell(`
...
export async function sendConfirmationEmail(
  ...
  const html = shell(`
    ...
    <table style="margin:16px 0;border-collapse:collapse">
      <tr><td style="padding:4px 12px 4px 0;color:#666">Termin</td><td style="padding:4px 0"><strong>${args.terminLabel}</strong></td></tr>
      <tr><td style="padding:4px 12px 4px 0;color:#666">Bei</td><td style="padding:4px 0">${args.arztName}</td></tr>
      ${args.address ? `<tr><td style="padding:4px 12px 4px 0;color:#666">Ort</td><td style="padding:4px 0">${args.address}</td></tr>` : ''}
    </table>
    <p style="margin:20px 0">${btn(args.icsUrl, 'Zum Kalender hinzufügen (.ics)')}</p>
